...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,
	...
	\node[initial,vertex]	(A)                    		{\tiny $IDLE$};
  	\node[vertex]         	(B) [below left=6cm of A ] 	{\tiny $CHECK1$};
  	\node[vertex]         	(C) [below left=4cm of B ] 	{\tiny $WRITEBACK$};
  	\node[vertex]         	(D) [below right=6cm of C] 	{\tiny $WRITE$};
  	\node[vertex]			(E) [below right=6cm of A]	{\tiny $CHECKREAD$};
  	\node[vertex]			(F) [below right=4cm of E]  {\tiny $WRITEBACKREAD$};
  	\node[vertex]			(G) [below left=4cm of F]  {\tiny $READ$};

	% ------------------------------------------------------------------------------
	% Definition of paths.
	% ------------------------------------------------------------------------------
	\path (A) [edge] 			  	edge [bend right] node 	{wrCPURequest/-} 						(B)
		  (B) [edge] 				edge node 				{lineIsDirty/stallCPU='1'} 				(C)
		  (B) [edge] 				edge [bend right] node 	{cacheHit/dataCPU2Cache, setDirty='1'} 	(A)
		  (B) [edge]				edge [bend right] node  {lineIsInvalid}							(D)
		  (B) [edge]				edge [bend left] node 	{lineIsNotDirty}						(D)
		  (C) [edge]				edge [bend right] node	{readyMEM}								(D)
		  (D) [edge]				edge [bend right] node 	{redayMEM}								(A);
