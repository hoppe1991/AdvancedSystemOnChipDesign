...
$ghdl -m -g -O3 --ieee=synopsys --workdir=$workDirectory $config mips_sim_fib_tb
echo "."
echo "++++++++++ run the executable for 15us and save all waveforms ++++++++++"
echo $tagFilename$
$ghdl -r -g -O3 --ieee=synopsys --workdir=$workDirectory $config mips_sim_fib_tb --stop-time=40us --wave=../sim/$asmFilename.ghw -gDFileName=$DFileName -gIFileName=$IFileName -gTAG_FILENAME=$tagFilename$ -gDATA_FILENAME=$dataFilename -gFILE_EXTENSION=$fileExtension
