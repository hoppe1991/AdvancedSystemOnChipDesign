...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,
	...
	% Definition of paths.
	% ------------------------------------------------------------------------------
	\path (A) [edge, midway, above, sloped] edge [bend right] node {wrCPU='1'/-} (B)
		  (B) [edge, midway, above, sloped] edge node {cacheMiss='1'/stallCPU='1'} (C)
		  (B) [edge, midway, above, sloped] edge [bend right] node {cacheHit='1'/dataCPU2Cache, setDirty='1'} (A)
		  (C) [edge, midway, above, sloped] edge node {isDirty='1'/wrMEM='1'} (D)
		  (D) [edge, midway, above, sloped] edge node {readyMEM='1'/rdMEM='1'} (E)
		  (E) [edge, midway, above, sloped] edge node {readyMEM='1'/dataCPU2Cache, setDirty='1'} (A);
		  
	\path (A) [edge, midway, above, sloped] edge [bend left] node {rdCPU='1'/-} (F)
		  (F) [edge, midway, above, sloped] edge node {cacheMiss='1'/stallCPU='1'} (G)
		  (F) [edge, midway, above, sloped] edge [bend left] node {cacheHit='1'/dataCPU} (A)
		  (G) [edge, midway, above, sloped] edge node {isDirty='1'/wrMEM='1'} (H)
		  (H) [edge, midway, above, sloped] edge node {readyMEM='1'/rdMEM='1'} (I)
		  (I) [edge, midway, above, sloped] edge node {readyMEM='1'/dataCPU} (A);
